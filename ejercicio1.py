...
class Alumno:
    ...
    def antiguedad(self):
        ...
        return tiempoCarrera.days // 365  # Cantidad de años en la carrera

    @staticmethod
    def generar_nombre():
        nombres = ["Franco", "Valentina", "Araceli", "Daniel", "Matias"]
    ...
    def generar_fecha_ingreso():
        start_date = datetime.now() - timedelta(days=365*4)  # Fecha hace máx. 4 años
        end_date = datetime.now()
        return start_date + (end_date - start_date) * random.random()

    @staticmethod
    def generar_carrera():
        carreras = ["Ed. Fisica", "Ingenieria", "Maestra jardinera", "Programador", "Psicología"]
        return random.choice(carreras)

# Función para guardar la lista de alumnos en un archivo
def guardar_lista_alumnos(lista_alumnos, directorio):
    # Crear el directorio si no existe
    if not os.path.exists(directorio):
        os.makedirs(directorio)
    
    # Guardar cada alumno como un diccionario en una lista
    lista_alumnos_dict = []
    for alumno in lista_alumnos:
        lista_alumnos_dict.append(alumno.datos)
    
    # Escribir la lista de alumnos en un archivo JSON dentro del directorio
    archivo = os.path.join(directorio, 'lista_alumnos.json')
    with open(archivo, 'w') as f:
        json.dump(lista_alumnos_dict, f, indent=4)
    
    print(f"Lista de alumnos guardada en {archivo}")

# Función para mover el directorio a una nueva ruta y borrar el archivo y el directorio
def mover_y_borrar(directorio, nueva_ruta):
    try:
        # Mover el directorio a la nueva ruta
        os.rename(directorio, nueva_ruta)
        print(f"Directorio movido a {nueva_ruta}")
        
        # Borrar el archivo dentro del directorio
        archivo = os.path.join(nueva_ruta, 'lista_alumnos.json')
        os.remove(archivo)
        print(f"Archivo {archivo} borrado")
        
        # Borrar el directorio vacío
        os.rmdir(nueva_ruta)
        print(f"Directorio {nueva_ruta} borrado")
    
    except FileNotFoundError:
        print("El archivo o directorio no encontrado.")
